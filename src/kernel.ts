...
export class PolyglottKernel implements IKernel {
    ...
    */
    ...
    protected async getKernelByName(kernelName: string): Promise<IKernel | undefined> {


        if (!this.startedKernels.has(kernelName)) {

            if(!this.kernelSpecs.factories.has(kernelName)) {
                console.warn(`kernel ${kernelName} not found, available kernels are:`, Array.from(this.kernelSpecs.factories.keys()));
                return undefined;
            }

            const factory = this.kernelSpecs.factories.get(kernelName)!;
            const sendMessage = (msg: KernelMessage.IMessage) => this._sendMessageDispatch(msg, kernelName);


            const options = {id: `${kernelName}-1`, name: kernelName, location: this._location, sendMessage: sendMessage};
            console.log('Creating kernel with options', options);
